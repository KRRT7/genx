...
def Refl(theta, lamda, n, d, sigma, return_int=True):
    ...
    sigma = sigma[:-1]
    k = 2 * math.pi / lamda
    cos_theta = cos(theta * math.pi / 180) ** 2
    n_ratio_sq = (n[:, np.newaxis] / n[-1]) ** 2
    Qj = 2 * n[-1] * k * np.sqrt(n_ratio_sq - cos_theta)

    sigma_sq = sigma[:, np.newaxis] ** 2
    rp = (
        (Qj[1:] - Qj[:-1])
        / (Qj[1:] + Qj[:-1])
        * np.exp(-0.5 * Qj[1:] * Qj[:-1] * sigma_sq)
    )

    p = np.exp(1.0j * d[:, np.newaxis] * Qj[1:-1])

    # Using zip directly with learned rint[0], rint[1] without creating a new array
    r = rp[0]
    for rp_val, p_val in zip(rp[1:], p):
        r = (rp_val + r * p_val) / (1 + r * rp_val * p_val)
